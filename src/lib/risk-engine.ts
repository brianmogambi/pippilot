...
//
// Risk Engine v2 — single source of truth for position-sizing and
// risk-evaluation rules. Pure, synchronous, backend-safe (no React,
// no Supabase, no DOM/Node-only globals). The only allowed import is
// pure helpers from ./pip-value, which is itself dependency-free.
//
// This module can be mirrored verbatim into supabase/functions/_shared/
// when an Edge Function needs to gate signals on risk in a future phase.

import { pipMultiplier } from "./pip-value";
...
export interface TradeInputs {
  ...
  entry: number;
  stopLoss: number;
  /** Pip value in USD per standard lot. Injected by caller — engine never resolves it. */
  pipValueUSD: number;
  riskMode: RiskMode;
  /** Used only when riskMode === "fixed" and value > 0. */
...
export interface DailyState {
  /** Realized loss as a positive number (0 when flat or in profit). */
  realizedLossUSD: number;
  /** Sum of money-at-risk across currently open positions. */
  openRiskUSD: number;
}
...
export interface RiskEvaluation {
  ...
  pipDistance: number;
  rawLotSize: number;
  /** After conservative-mode halving. */
  lotSize: number;
...
// ── Placeholder: correlated exposure ────────────────────────────
...
/** No-op: never emits a warning. Replace in a future phase. */
...
/** No-op: returns { passed: true, warnings: [] } when rules is null. */
export function evaluatePropFirmRules(
  ...
  // Placeholder: real implementation lands in a future phase. The
  // shape is stable so call sites compile against it today.
...
export function evaluateTrade(args: {
  ...
  // Placeholder checks — currently no-ops, but the wiring is real so a
  // future phase fills in the bodies without touching call sites.
