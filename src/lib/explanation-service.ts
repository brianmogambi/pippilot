// src/lib/explanation-service.ts
//
// Phase 8 — Explanation Layer Hardening.
//
// Pure (apart from one fetch call), backend-safe service that owns the
// entire AI/fallback decision for signal explanations. The runner imports
// `generateExplanation()` and gets back text + metadata; it never sees
// the prompt, the parser, or the fallback logic.
//
// Backend-safe constraints (same as Phases 6 and 7):
// - No React, no Supabase, no DOM/Node-only globals.
// - Uses the global `fetch` (overridable via `fetchFn` for tests).
// - This file is mirrored verbatim into supabase/functions/_shared/
//   with the only difference being the explicit `.ts` extension on
//   relative imports (Deno requires it). There are no relative imports
//   today, so the mirror is a true verbatim copy.
...
export function buildSystemPrompt(version: string = PROMPT_VERSION): string {
  switch (version) {
    ...
    default:
      // Unknown version → fall through to current. The metadata column
      // still records the requested version so the drift is auditable.
      return AI_SYSTEM_PROMPT_V1;
  }
}
...
function successResult(
  ...
  return {
    ...
    // For no_trade signals the AI may emit a reason; keep the fallback
    // value otherwise so the column is always populated.
