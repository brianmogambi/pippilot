// supabase/functions/_shared/alert-engine.ts
//
// Deno mirror of src/lib/alert-engine.ts — keep in sync. Only difference
// from the src/lib copy is the explicit `.ts` extensions on relative
// imports (Deno requires them).

import { isJpyPair } from "./pip-value.ts";
import {
...
function entryTolerance(pair: string, atr?: number): number {
  const fivePips = isJpyPair(pair) ? 0.05 : 0.0005;
